...
void read_graph(char* graph)
{
    ...
    vbits1 = new vector<vector<bool> >(n, vector<bool>(n));
    nodes.resize(n);
    node2num.clear();
    input >> node1 >> node2;
    while(input)
...
void com_edges()
{
        ...
        for(int j=i+1; j<num_cliques; j++)
        {
            ...
            for(int k=0; k<cliques[i].size(); k++)
            {
                for(int s=0; s<cliques[j].size(); s++)
                {
                    ...
                    {
                        counter++;
                        clique_a.push_back(cliques[i][k]);
                        clique_b.push_back(cliques[j][s]);
                    }
                }
            }
            if(counter >= edges_t)
            {
                ...
                {
                    (*vbits1)[clique_a[s]][clique_b[s]] = 1;
                    (*vbits1)[clique_b[s]][clique_a[s]] = 1;
                }
            }
        }
    }
    ...
    for(int i=0; i<num_cliques; i++)
        for(int k=0; k<cliques[i].size()-1; k++)
            for(int j=k+1; j<cliques[i].size(); j++)
            {
                (*vbits1)[cliques[i][k]][cliques[i][j]] = 1;
                (*vbits1)[cliques[i][j]][cliques[i][k]] = 1;
            }
